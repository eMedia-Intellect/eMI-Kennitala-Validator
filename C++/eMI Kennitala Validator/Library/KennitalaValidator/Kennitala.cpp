...
#include <string>
#include "Kennitala.hpp"

namespace KennitalaValidator
{
	Kennitala::Type Kennitala::Validate(std::string const &kennitala)
	{
		// The national identification number can only contain digits.
		if (kennitala.find_first_not_of("0123456789") == std::string::npos)
		{
			...
			if (kennitala.length() == 10)
			{
				...
				if ((digit1 >= 0 && digit1 <= 7) && (digit2 >= 0 && digit2 <= 9) && (digit3 == 0 || digit3 == 1) && (digit4 >= 0 && digit4 <= 9) && (digit5 >= 0 && digit5 <= 9) && (digit6 >= 0 && digit6 <= 9) && (digit7 >= 0 && digit7 <= 9) && (digit8 >= 0 && digit8 <= 9) && (digit9 >= 0 && digit9 <= 9) && (digit10 == 0 || digit10 == 8 || digit10 == 9))
				{
					...
					if (checkDigit == digit9)
					{
						...
						{
							return Kennitala::Type::Organisation;
						}

						return Kennitala::Type::Individual;
					}
				}
			}
		}
